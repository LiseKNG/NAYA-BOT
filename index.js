...
bot.command("menu", async (ctx) => {
  registerChat(ctx.chat.id, ctx.chat.type);
  await ctx.reply("🌌 <b>Menu de Naya</b> 🌌\n\nChoisis une option :", {
    parse_mode: "HTML",
...
bot.action("menu_leaderboard", async (ctx) => {
  ...
  const board = getLeaderboard(ctx.chat.id, "total").slice(0, 10);

  if (board.length === 0) {
    await ctx.reply("Aucune activité enregistrée pour l'instant dans ce chat.");
    return;
  }
  ...
  try {
    const imageBuffer = await generateLeaderboardImage(board, "Classement général");
    await ctx.replyWithPhoto(
      { source: imageBuffer },
      { caption: `🏆 Classement général :\n${textList}` }
    );
  } catch (err) {
    console.error("Erreur image classement (menu):", err.message);
    await ctx.reply(`🏆 Classement général :\n${textList}`);
  }
});

bot.action("menu_game_info", async (ctx) => {
  await ctx.answerCbQuery();
  await ctx.reply(
    "🔤 Le jeu \"devine le mot\" se lance automatiquement dans les groupes de temps en temps ! Reste attentif, la première bonne réponse tapée dans le chat gagne des étoiles ⭐"
  );
});

bot.action("menu_music", async (ctx) => {
  await ctx.answerCbQuery();
  await ctx.reply("🎵 Dis-moi le titre d'une chanson (et l'artiste si tu veux), je te trouve ça tout de suite !");
});

bot.action("menu_stickers", async (ctx) => {
  await ctx.answerCbQuery();
  await ctx.reply(
    "😊 Pas besoin de le demander — je t'envoie un sticker naturellement selon mon humeur pendant qu'on discute !"
  );
});
...
bot.command("dashboard", async (ctx) => {
  ...
  if (!publicUrl) {
    await ctx.reply("Le tableau de bord n'est pas encore configuré (PUBLIC_URL manquant).");
    return;
  }
  await ctx.reply("📊 Tableau de bord du groupe :", {
    reply_markup: {
      inline_keyboard: [
...
// Filet de sécurité global : une erreur dans un seul handler ne doit jamais
// faire planter tout le bot. Sans ça, une erreur Telegram inattendue (comme
// un type de bouton refusé) coupe complètement Naya jusqu'au redémarrage.
bot.catch((err, ctx) => {
  console.error(`Erreur non gérée pour la mise à jour ${ctx.updateType}:`, err.message);
});

bot.launch();
startDashboardServer(bot);
